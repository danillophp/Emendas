...
class MasterController extends Controller
{
    ...
    public function updateDemand(): void
    {
        ...
        $changedFields = [];
        foreach (['emenda','nome_politico','numero_processo_sei','tipo_processo','tipo_processo_outros','tipo_emenda','tipo_emenda_outros','data_prazo_resposta','data_cadastro_emenda','funcionario_id'] as $field) {
    ...
    private function validateDemandPayload(?string $existingAttachment = null): array
    {
        $data = [
            ...
            'tipo_emenda_outros' => trim((string)($_POST['tipo_emenda_outros'] ?? '')),
        ...
        if ($data['tipo_emenda'] === 'outros') {
            if ($data['tipo_emenda_outros'] === '') {
                $errors[] = 'Informe o tipo de emenda quando selecionar "Outros".';
            } elseif (mb_strlen($data['tipo_emenda_outros']) > 255) {
                $errors[] = 'O tipo de emenda personalizado deve ter no máximo 255 caracteres.';
            }
        } else {
            $data['tipo_emenda_outros'] = null;
        }
