...
<form method="post" action="<?= url('master/demands/create') ?>" class="row g-3" enctype="multipart/form-data">
  ...
  <div class="col-md-4"><select class="form-select js-tipo-processo" name="tipo_processo" required><option value="">Tipo de demanda</option><?php foreach ($processLabels as $k=>$l): ?><option value="<?= $k ?>"><?= $l ?></option><?php endforeach; ?></select></div>
  <div class="col-md-4 d-none js-tipo-processo-outros-wrap"><input class="form-control js-tipo-processo-outros" name="tipo_processo_outros" placeholder="Especifique o tipo de demanda" maxlength="255"></div>
  <div class="col-md-3"><select class="form-select js-tipo-emenda" name="tipo_emenda" required><option value="">Tipo de emenda</option><?php foreach ($amendmentLabels as $k=>$l): ?><option value="<?= $k ?>"><?= $l ?></option><?php endforeach; ?></select></div>
  <div class="col-md-4 d-none js-tipo-emenda-outros-wrap"><input class="form-control js-tipo-emenda-outros" name="tipo_emenda_outros" placeholder="Especifique o tipo de emenda" maxlength="255"></div>
  <div class="col-md-4">
...
<tr>
  <td><strong><?= e($demand['emenda']) ?></strong><br><small class="text-muted"><?= e((($demand['tipo_processo'] ?? '') === 'outros' && trim((string)($demand['tipo_processo_outros'] ?? '')) !== '') ? $demand['tipo_processo_outros'] : ($processLabels[$demand['tipo_processo']] ?? $demand['tipo_processo'])) ?> • <?= e((($demand['tipo_emenda'] ?? '') === 'outros' && trim((string)($demand['tipo_emenda_outros'] ?? '')) !== '') ? $demand['tipo_emenda_outros'] : ($amendmentLabels[$demand['tipo_emenda']] ?? (($demand['tipo_emenda'] ?? '') === 'parlamentar' ? 'Federal' : $demand['tipo_emenda']))) ?></small><br><small class="text-muted">SEI: <?= e(trim((string)($demand['numero_processo_sei'] ?? '')) !== '' ? $demand['numero_processo_sei'] : 'Não informado') ?></small></td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row g-3 mb-3">
          ...
          <div class="col-md-4"><small class="text-muted d-block">Tipo de demanda</small><strong><?= e((($demand['tipo_processo'] ?? '') === 'outros' && trim((string)($demand['tipo_processo_outros'] ?? '')) !== '') ? $demand['tipo_processo_outros'] : ($processLabels[$demand['tipo_processo']] ?? $demand['tipo_processo'])) ?></strong></div>
          <div class="col-md-4"><small class="text-muted d-block">Tipo de emenda</small><strong><?= e((($demand['tipo_emenda'] ?? '') === 'outros' && trim((string)($demand['tipo_emenda_outros'] ?? '')) !== '') ? $demand['tipo_emenda_outros'] : ($amendmentLabels[$demand['tipo_emenda']] ?? (($demand['tipo_emenda'] ?? '') === 'parlamentar' ? 'Federal' : $demand['tipo_emenda']))) ?></strong></div>
...
<div class="modal fade" id="editar<?= (int)$demand['id'] ?>" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Editar demanda</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
    <form method="post" action="<?= url('master/demands/update') ?>" enctype="multipart/form-data"><div class="modal-body row g-3">
      ...
      <div class="col-md-4"><label class="form-label">Tipo de demanda</label><select class="form-select js-tipo-processo" name="tipo_processo" required><?php foreach ($processLabels as $k=>$l): ?><option value="<?= $k ?>" <?= $demand['tipo_processo']===$k?'selected':'' ?>><?= $l ?></option><?php endforeach; ?></select></div>
      <div class="col-md-4 <?= ($demand['tipo_processo'] ?? '') === 'outros' ? '' : 'd-none' ?> js-tipo-processo-outros-wrap"><label class="form-label">Tipo de demanda (Outros)</label><input class="form-control js-tipo-processo-outros" name="tipo_processo_outros" maxlength="255" value="<?= e($demand['tipo_processo_outros'] ?? '') ?>"></div>
      <div class="col-md-4"><label class="form-label">Tipo de emenda</label><select class="form-select js-tipo-emenda" name="tipo_emenda" required><?php foreach ($amendmentLabels as $k=>$l): ?><option value="<?= $k ?>" <?= $demand['tipo_emenda']===$k?'selected':'' ?>><?= $l ?></option><?php endforeach; ?></select></div>
      <div class="col-md-4 <?= ($demand['tipo_emenda'] ?? '') === 'outros' ? '' : 'd-none' ?> js-tipo-emenda-outros-wrap"><label class="form-label">Tipo de emenda (Outros)</label><input class="form-control js-tipo-emenda-outros" name="tipo_emenda_outros" maxlength="255" value="<?= e($demand['tipo_emenda_outros'] ?? '') ?>"></div>
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('form').forEach(function (form) {
    ...
    var selectEmenda = form.querySelector('.js-tipo-emenda');
    var wrapEmenda = form.querySelector('.js-tipo-emenda-outros-wrap');
    var inputEmenda = form.querySelector('.js-tipo-emenda-outros');
    if (selectEmenda && wrapEmenda && inputEmenda) {
      var syncEmenda = function () {
        var isOutrosEmenda = selectEmenda.value === 'outros';
        wrapEmenda.classList.toggle('d-none', !isOutrosEmenda);
        inputEmenda.required = isOutrosEmenda;
        if (!isOutrosEmenda) inputEmenda.value = '';
      };
      selectEmenda.addEventListener('change', syncEmenda);
      syncEmenda();
    }
  });
});
