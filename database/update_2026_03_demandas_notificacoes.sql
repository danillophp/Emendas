...
BEGIN
    ...
    IF v_exists = 0 THEN
        ALTER TABLE demandas
          ADD COLUMN tipo_processo ENUM('prestacao_de_conta','cadastro_de_emenda','outros') NULL AFTER nome_politico;
    END IF;
    ...
    ALTER TABLE demandas
      MODIFY COLUMN tipo_processo ENUM('prestacao_de_conta','cadastro_de_emenda','outros') NULL;
    ...
    IF v_exists = 0 THEN
        ALTER TABLE demandas
          ADD COLUMN tipo_emenda ENUM('federal','estadual','municipal','outros') NULL AFTER tipo_processo_outros;
    ELSE
        -- Mantém dados existentes e converte tipo
        ALTER TABLE demandas
          MODIFY COLUMN tipo_emenda ENUM('federal','estadual','municipal','outros') NULL;
    END IF;


    -- 1.2.1 tipo_emenda_outros
    SELECT COUNT(*) INTO v_exists
      FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE()
       AND TABLE_NAME = 'demandas'
       AND COLUMN_NAME = 'tipo_emenda_outros';

    IF v_exists = 0 THEN
        ALTER TABLE demandas
          ADD COLUMN tipo_emenda_outros VARCHAR(255) NULL AFTER tipo_emenda;
    END IF;

    -- garante enum atualizado com opção "outros"
    ALTER TABLE demandas
      MODIFY COLUMN tipo_emenda ENUM('federal','estadual','municipal','outros') NULL;
    ...
    ALTER TABLE demandas
      ...
      MODIFY COLUMN tipo_processo ENUM('prestacao_de_conta','cadastro_de_emenda','outros') NOT NULL,
      MODIFY COLUMN tipo_emenda ENUM('federal','estadual','municipal','outros') NOT NULL,
